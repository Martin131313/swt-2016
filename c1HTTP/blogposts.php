...
if(isset($_GET['id'])){
	...
	$query_bp->execute();
	$res_db = $query_bp->fetchAll(PDO::FETCH_ASSOC);

	$input_title = $res_db[0]['title'];
	$input_content = $res_db[0]['content'];
	$input_date = $res_db[0]['publish_date'];
	$input_id_category = $res_db[0]['id_category'];
	$input_image = $res_db[0]['image'];
} else {
	echo 'INSERT MODE';
...
$query_blogpost = $db->prepare($sql_blogposts);
$query_blogpost->execute();
$query_blogpost_result = $query_blogpost->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<table width="100%" border="1">
		...
		<?php foreach($query_blogpost_result as $row){ ?>
		<tr>
			<td><?=$row['title']?></td>
			<td><?=$row['content']?></td>
